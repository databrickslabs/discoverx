...
class DiscoveryExecutor:
    def __init__(self, spark: SparkSession, scan_results: ScanResult, from_tables="*.*.*"):
        self.spark = spark
        self._scan_result = scan_results
    ...
    def search_by_class(
        ...
        Msql.validate_from_components(self.from_tables)
        ...
        return self._msql(
            f"SELECT {select_statement}, to_json(struct(*)) AS row_content FROM {self.from_tables} {where_statement}",
            min_score=min_score,
        )
    ...
    def select_by_classes(
        ...
        return self._msql(
            f"SELECT {from_statement}, to_json(struct(*)) AS row_content FROM {self.from_tables}",
            min_score=min_score,
        )
    ...
    def delete_by_class(
        ...
        if not yes_i_am_sure:
            logger.friendly(
                f"Please confirm that you want to delete the following values from the table {self.from_tables} using the class {by_class}: {values}"
            )
            logger.friendly(
        ...
        delete_result = self._msql(
            f"DELETE FROM {self.from_tables} WHERE `[{by_class}]` IN ({value_string})",
