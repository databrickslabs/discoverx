...
class DX:
    ...
    def _can_read_columns_table(self) -> bool:
        try:
            self.spark.sql(
                f"SELECT * FROM {self.COLUMNS_TABLE_NAME} WHERE table_catalog = 'system' LIMIT 1"
            )
            return True
        except Exception as e:
            self.logger.error(
                f"Error while reading table {self.COLUMNS_TABLE_NAME}: {e}"
            )
            return False
    ...
    def from_tables(self, from_tables: str = "*.*.*"):
        ...
        return DataExplorer(
            from_tables, self.spark, InfoFetcher(self.spark, self.COLUMNS_TABLE_NAME)
        )
